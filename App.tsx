...
const App: React.FC = () => {
  ...
  // Stress-Driven Biofeedback Effect
  useEffect(() => {
    const { stressLogs } = stressDataHook;
    if (stressLogs.length > 0) {
        const sortedLogs = [...stressLogs].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        const latest = sortedLogs[0];
        const isToday = new Date(latest.timestamp).toDateString() === new Date().toDateString();
        
        // If latest log is from today and level is high (>= 8), trigger biofeedback glow
        if (isToday && latest.level >= 8) {
            setIsHighStress(true);
        } else {
            // Note: This might override RealtimeStress temporary pulse if logs change during pulse, 
            // but ensures consistent state based on recorded data.
            setIsHighStress(false);
        }
    }
  }, [stressDataHook.stressLogs]);
    ...
    switch (activeView) {
      ...
      case 'admin':
        if (!isAdminAuthenticated) {
            return <AdminLogin onLoginSuccess={() => setIsAdminAuthenticated(true)} onBack={() => setActiveView('dashboard')} />;
        }
  ...
  return (
    <div className={`flex flex-col sm:flex-row min-h-screen bg-background font-sans text-foreground transition-all duration-1000 relative ${isHighStress ? 'biofeedback-glow' : ''}`}>
      {/* Background Animation sits behind everything */}
      <BackgroundAnimation />

      {/* Sidebar Navigation for Desktop - Sticky and Scrollable */}
      <aside className="hidden sm:flex flex-col w-64 bg-card p-4 border-r border-border z-10 sticky top-0 h-screen overflow-y-auto">
        ...
        <div className="flex flex-col space-y-2 pt-4 border-t border-border shrink-0">
      ...
      <div className="flex-1 flex flex-col z-10 relative">
        <header className="flex items-center justify-between p-4 bg-card border-b border-border shrink-0 sticky top-0 z-20">
        ...
        {/* Main Content Area - Full height scrolling */}
        <main className="flex-1 flex flex-col">
          <div className="flex-1 p-2 sm:p-6 w-full max-w-[1920px] mx-auto">
            {renderView()}
          </div>
      ...
      {/* Floating Chat Widget - Available globally except when main view is chat */}
      {activeView !== 'chat' && (
        <FloatingChat 
      ...
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border p-1 grid grid-cols-5 z-20">
