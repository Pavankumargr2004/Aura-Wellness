...
import { ArrowLeft, BookOpen, Loader2, Heart, RefreshCw, Dices, CloudRain, Waves, Leaf, Volume2, Square } from 'lucide-react';
import { useLanguage } from '../../contexts/LanguageContext';
import { type CalmTale, type StoryCategory, type StoryMood } from '../../types';
import { generateCalmTales, generateSpeech } from '../../services/geminiService';
...
const StoryReaderView: React.FC<{
    ...
    const handleReadAloud = async () => {
        ...
        const textToRead = `${story.title}. by ${story.author}. ${story.content}`;
            ...
            <div className="flex items-center justify-between p-4 bg-transparent">
                ...
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        className={`p-2 rounded-full transition-colors ${isReadingAloud ? 'text-primary bg-primary/20' : 'text-muted-foreground hover:bg-secondary'}`}
                        title={isReadingAloud ? "Stop Reading" : "Read Aloud"}
                    >
                        {isLoadingTTS ? <Loader2 className="h-5 w-5 animate-spin"/> : isReadingAloud ? <Square className="h-5 w-5 fill-current"/> : <Volume2 className="h-5 w-5" />}
                    </button>
