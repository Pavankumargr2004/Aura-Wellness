...
const DreamAnalyzer: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="glass-card h-full rounded-2xl p-4 sm:p-6 flex flex-col animate-fadeIn max-w-3xl mx-auto">
      <button
        onClick={onBack}
        className="self-start mb-4 text-primary hover:text-violet-300 font-semibold transition-colors flex items-center gap-2"
      >
      ...
      <textarea
        ...
        className="w-full flex-1 p-3 text-sm bg-secondary rounded-lg text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary min-h-[150px]"
        disabled={isLoading}
      />
      ...
      <button
        ...
        className="mt-4 w-full py-3 px-4 rounded-lg font-semibold text-white bg-primary hover:bg-violet-700 transition-colors disabled:bg-muted flex items-center justify-center gap-2"
      >
        {isLoading ? (
          <>
            <Loader2 className="animate-spin h-5 w-5" />
            {t("tool_dream_analyzing")}
          </>
        ) : (
          t("tool_dream_button")
        )}
      ...
      {!imageResult && !isLoading && (
        <div className="flex-1 flex items-center justify-center">
          <button
            ...
            className="py-4 px-8 text-lg rounded-lg font-semibold text-white bg-primary hover:bg-violet-700 transition-colors flex items-center justify-center gap-3 shadow-lg shadow-primary/20"
          >
            <Sparkles />
            {t("tool_calm_button")}
          </button>
      ...
      {imageResult && !isLoading && (
        <div className="flex-1 flex flex-col items-center justify-center animate-fadeIn">
          <div className="w-full aspect-video rounded-lg overflow-hidden shadow-lg border-2 border-primary/50">
            <img
              src={`data:image/jpeg;base64,${imageResult.base64Image}`}
              alt={imageResult.prompt}
              className="w-full h-full object-cover"
            />
          </div>

          <p className="text-xs text-muted-foreground italic mt-2 text-center max-w-xl">
            "{imageResult.prompt}"
          </p>

          <button
            onClick={handleGenerate}
            className="mt-6 py-2 px-6 rounded-lg font-semibold text-white bg-secondary hover:bg-muted transition-colors"
          >
...
const ToolCard = ({
  ...
  <button
    onClick={onClick}
    className="glass-card p-8 rounded-3xl text-left w-full h-full flex flex-col transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl border border-white/5 group relative overflow-hidden"
  >
    ...
    <div className="mb-6 rounded-2xl p-4 w-max bg-secondary/50 ring-1 ring-white/10 group-hover:bg-primary/20 group-hover:ring-primary/30 transition-all">
    ...
    <div className="mt-6 w-8 h-8 rounded-full bg-secondary flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="text-center">
        ...
        <h2 className="text-4xl font-bold mb-4">{t("games_title")}</h2>
...
const MainView = ({ onSelect }: { onSelect: (view: View) => void }) => {
  ...
  return (
    <div className="space-y-12 pb-12">
      <div className="text-center">
        <h2 className="text-4xl font-bold mb-4">{t("tools_title")}</h2>
...
const Tools: React.FC<ToolsProps> = ({ stressDataHook }) => {
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case "emotion_wheel":
        return (
          <EmotionWheel onBack={handleBack} stressDataHook={stressDataHook} />
        );
      case "dream_analyzer":
      ...
      case "games":
        return (
          <MiniGamesView
            onSelectGame={(game) => setView(game as View)}
            onBack={() => setView("main")}
          />
        );
      default:
