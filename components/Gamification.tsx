...
const Gamification: React.FC<{ stressDataHook: StressDataHook }> = ({ stressDataHook }) => {
    ...
    const handleCompleteChallenge = () => {
        ...
        const cleanedChallenge = cleanChallengeText(challenge);
        addWellnessActivity(`Completed Challenge: ${cleanedChallenge.substring(0, 50)}${cleanedChallenge.length > 50 ? '...' : ''}`);
        setIsCompleted(true);
            ...
            <div className="glass-card p-6 rounded-2xl">
                ...
                {isLoading ? (
                    ...
                    <div className="bg-primary/20 border-l-4 border-primary p-4 rounded-r-lg">
                        <p className="text-primary-foreground font-medium">"{cleanChallengeText(challenge)}"</p>
                    </div>
                )}
                <button
