...
import { SlidersHorizontal, Trash2, Bot, BellRing, Shield, Globe, Download, Eye, Sun, Moon, Activity, Smartphone, Check, Loader2 } from 'lucide-react';
...
const Settings: React.FC<SettingsProps> = ({ stressDataHook, chatHistoryHook, chatSettingsHook, remindersHook, setActiveView }) => {
    ...
    // Simulated Health Data Integration State
    const [healthProvider, setHealthProvider] = useState<'apple' | 'google' | null>(null);
    ...
    const connectHealthProvider = (provider: 'apple' | 'google') => {
        setIsSyncing(true);
        // Simulate API delay
        setTimeout(() => {
    ...
    const disconnectHealthProvider = () => {
        // Immediate disconnect toggle
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-fadeIn">
            ...
            <div className="glass-card p-6 rounded-2xl">
                <h3 className="text-xl font-bold mb-4 flex items-center gap-3"><Eye className="h-6 w-6 text-primary"/> Appearance</h3>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                    ...
                    <button
                        onClick={toggleTheme}
                        className="mt-4 sm:mt-0 flex items-center gap-2 py-2 px-4 rounded-lg font-semibold bg-secondary hover:bg-muted text-foreground transition-colors"
                    >
                       {theme === 'dark' ? <><Moon className="h-4 w-4"/> {t('darkMode')}</> : <><Sun className="h-4 w-4"/> {t('lightMode')}</>}
                    </button>
                </div>
            </div>

            <div className="glass-card p-6 rounded-2xl">
                <h3 className="text-xl font-bold mb-4 flex items-center gap-3"><Activity className="h-6 w-6 text-primary"/> Health Integrations</h3>
                <p className="text-sm text-muted-foreground mb-6">Automatically sync steps, sleep, and heart rate data to enhance your Live Meter accuracy.</p>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className={`p-4 rounded-xl border transition-all ${healthProvider === 'apple' ? 'bg-primary/10 border-primary' : 'bg-secondary/30 border-transparent hover:border-white/10'}`}>
                        <div className="flex justify-between items-start mb-3">
                            ...
                            {healthProvider === 'apple' && <Check className="h-5 w-5 text-primary" />}
                        </div>
                        <p className="text-xs text-muted-foreground mb-4">Syncs with HealthKit on iOS devices.</p>
                        {healthProvider === 'apple' ? (
                            <button 
                                onClick={disconnectHealthProvider} 
                                className="w-full py-2 px-3 rounded-lg border border-red-500/50 text-red-500 text-xs font-bold hover:bg-red-500/10 transition-colors"
                            >
                                Disconnect
                            </button>
                        ) : (
                            <button 
                                onClick={() => connectHealthProvider('apple')} 
                                disabled={isSyncing || healthProvider !== null}
                                className="w-full py-2 px-3 rounded-lg bg-white text-black text-xs font-bold hover:bg-gray-100 transition-colors disabled:opacity-50"
                            >
                                {isSyncing ? <Loader2 className="h-3 w-3 animate-spin mx-auto"/> : 'Connect'}
                            </button>
                        )}
                    </div>

                    <div className={`p-4 rounded-xl border transition-all ${healthProvider === 'google' ? 'bg-primary/10 border-primary' : 'bg-secondary/30 border-transparent hover:border-white/10'}`}>
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-white text-black rounded-lg">
                            ...
                            {healthProvider === 'google' && <Check className="h-5 w-5 text-primary" />}
                        </div>
                        <p className="text-xs text-muted-foreground mb-4">Syncs with Google Fit on Android.</p>
                        {healthProvider === 'google' ? (
                            ...
                            <button 
                                onClick={() => connectHealthProvider('google')} 
                                disabled={isSyncing || healthProvider !== null}
                                className="w-full py-2 px-3 rounded-lg bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 transition-colors disabled:opacity-50"
                            >
                                {isSyncing ? <Loader2 className="h-3 w-3 animate-spin mx-auto"/> : 'Connect'}
                            </button>
                        )}
                ...
                {healthProvider && (
                    <div className="mt-6 flex items-center justify-between border-t border-border pt-4">
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">Keep your wellness data up to date automatically.</p>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" checked={autoSync} onChange={() => setAutoSync(!autoSync)} className="sr-only toggle-switch-input" />
