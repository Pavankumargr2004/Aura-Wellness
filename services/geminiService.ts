...
const commandSchema = {
    ...
    properties: {
        command: {
            type: Type.STRING,
            description: "The command to execute. One of: LOG_STRESS, LOG_SLEEP, LOG_EVENT, ANALYZE_BURNOUT, GET_TREND_REPORT, WELLNESS_ACTIVITY, ROLEPLAY_THERAPIST, GENERAL_CHAT, FIND_PLACES.",
        },
        payload: {
            type: Type.OBJECT,
            properties: {
                ...
                query: { type: Type.STRING, description: "Search query for FIND_PLACES, e.g., 'pharmacy nearby' or 'dentist'."},
            },
        },
        commentary: {
            type: Type.STRING,
...
export const processUserPrompt = async (
    ...
    try {
        ...
        Available commands: LOG_STRESS, LOG_SLEEP, LOG_EVENT, ANALYZE_BURNOUT, GET_TREND_REPORT, WELLNESS_ACTIVITY, ROLEPLAY_THERAPIST, GENERAL_CHAT, FIND_PLACES.
        ...
        If the user asks to find a physical place (e.g., 'find a pharmacy', 'nearest hospital'), use the FIND_PLACES command with the search query in the payload.
        ...
        Example 2: User says "Where is the nearest hospital?"
        Your response: {"command": "FIND_PLACES", "payload": {"query": "hospital"}, "commentary": "I can help you find that. Here are some hospitals nearby."}
        `;
...
export const analyzeMedicalCondition = async (base64Image: string): Promise<{ condition: string; solution: string; disclaimer: string }> => {
    try {
        const systemInstruction = `You are a helpful medical assistant AI. Your task is to analyze the provided image (which may show a skin condition, wound, or other visible symptom) and provide a preliminary assessment.
        
        STRUCTURE YOUR RESPONSE AS JSON:
        {
            "condition": "Name of the potential condition or observation.",
            "solution": "Suggested home remedies, immediate actions, or advice (e.g., 'Keep it clean', 'Apply cold compress'). Keep this concise and actionable.",
            "disclaimer": "Standard medical disclaimer emphasizing you are an AI and this is not professional medical advice. Recommend seeing a doctor."
        }

        Important:
        - If the image is not medical or unclear, state that in the "condition" field.
        ...
        const responseSchema = {
            ...
            required: ["condition", "solution", "disclaimer"],
        };

        const response = await ai.models.generateContent({
            model: 'gemini-3-pro-preview', // High reasoning capability for medical images
            contents: {
                role: 'user',
                parts: [
                    imagePart,
                    { text: "Analyze this image for any medical condition and provide advice." }
                ]
            },
            config: {
                ...generationConfig,
        ...
        return {
            ...
            disclaimer: "System error. Please consult a professional."
        };
    }
};
...
export const generateCalmSpaceImage = async (): Promise<CalmSpaceImage> => {
    try {
        ...
        // Use gemini-2.5-flash-image for generation (Nano Banana powered)
        const response = await ai.models.generateContent({
 ...
 * @returns AudioBuffer of the generated speech.
 */
 ...
 * @param query The user's search query (e.g., "hospitals nearby").
 * @param location Optional user location { lat, lng }.
 * @returns An object containing the text response and grounding metadata.
 */
export const findNearbyPlaces = async (
    ...
    try {
        ...
        // If location is available, add it to the retrieval config
        if (location) {
            config.toolConfig = {
                retrievalConfig: {
                    latLng: {
...
// --- Audio Decoding Helpers (Required for TTS) ---
